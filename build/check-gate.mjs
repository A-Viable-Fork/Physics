// Role: the v3 gate kernel's oracle (intake data model v3), adapted from
//   upstream/epistack/build/check-gate.mjs at the pinned commit for this repo's vendored layout.
//   Runs the acceptance suite phase by phase; the tests are the deliverable as much as the code,
//   because the core is trusted only because it is mechanically checkable. Phase A: the canonical
//   form, the records, the confidence order, the reference tables. This is a substrate-behavior
//   oracle per docs/status-ledger.md's oracle policy: it runs against the vendored kernel modules
//   themselves, self-contained, needing no corpus, so it verifies non-drift beyond hash matching.
// Contract: `node build/check-gate.mjs`. Exits 0 if every assertion holds, 1 with a report. Imports
//   only vendored kernel v3 modules and Node's standard library.
// Invariant: read-only; asserts determinism (byte-exact canonical forms, no float on the path) and
//   the spec's exact record fields and allowed values. DEPARTURE from the vendored original: every
//   "../kernel/..." import and "kernel/..." file-path literal is rewritten to
//   "../vendor/kernel/..." and "vendor/kernel/...", since this repo vendors the substrate under
//   vendor/ rather than holding it at the repo root. No assertion differs from the vendored file;
//   diffable against upstream/epistack/build/check-gate.mjs at the pinned commit to confirm only
//   paths changed.
import { readFileSync, readdirSync, statSync } from "node:fs";
import { dirname, join, relative } from "node:path";
import { fileURLToPath } from "node:url";
import { canonicalize, encode, hashOf, isExactDecimal } from "../vendor/kernel/schema/canonical.mjs";
import * as CONF from "../vendor/kernel/schema/confidence.mjs";
import * as R from "../vendor/kernel/schema/records.mjs";
import { makeSourceTable, makeKindTable, ceilingFor, footprintClosure } from "../vendor/kernel/schema/tables.mjs";
...
// --- A.3 exact-decimal round-trips verbatim; nothing parses a stored value to a float ---
{
  ...
  const v3 = ["vendor/kernel/schema/canonical.mjs", "vendor/kernel/schema/confidence.mjs", "vendor/kernel/schema/records.mjs", "vendor/kernel/schema/tables.mjs",
    "vendor/kernel/grounding/earned-grade.mjs", "vendor/kernel/gate/gate.mjs", "vendor/kernel/gate/verify.mjs", "vendor/kernel/store/state.mjs", "vendor/kernel/store/apply.mjs", "vendor/kernel/store/decay.mjs"];
...
// ================= PHASE C =================
import { genesis, makeState, verifyChain } from "../vendor/kernel/store/state.mjs";
import { apply, inForce } from "../vendor/kernel/store/apply.mjs";
import { computeDecay, derivedGrade } from "../vendor/kernel/store/decay.mjs";
...
// ================= PHASE D =================
import { verifyDecision, windowReplay, resubmissionCheck } from "../vendor/kernel/gate/verify.mjs";
...
{
  const v3 = ["vendor/kernel/schema/canonical.mjs", "vendor/kernel/schema/confidence.mjs", "vendor/kernel/schema/records.mjs", "vendor/kernel/schema/tables.mjs", "vendor/kernel/grounding/earned-grade.mjs", "vendor/kernel/gate/gate.mjs", "vendor/kernel/gate/verify.mjs", "vendor/kernel/store/state.mjs", "vendor/kernel/store/apply.mjs", "vendor/kernel/store/decay.mjs"];
